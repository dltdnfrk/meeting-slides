...
### Interaction-shell decision: Caret dual shell (web)

After black-box computer-use research on Caret/Aside, the operator interaction shell adopts a
**dual-mode web translation** (not a native CEF clone and not a brand copy):

1. **Library shell** — Meetings rail + meeting chrome (title/properties) + detail tabs
   (`Overview` / `Notes` / `Transcript`) + Overview sections (`Summary`, `Actions` follow-up card).
2. **Live split panel** — when `.app--capturing` is active, the shell leaves library mode and
   becomes a centered Caret-like panel with a fixed **slides | transcript** split
   (`#stage-pane` + `#splitter-transcript` + `#transcript-pane`). Meetings rail, glance,
   overview actions, and dense tool rows hide. Sticky Stop/timer, LIVE title marker, and the
   floating capture pill remain. `#current-slide` keeps rendering inside the stage. On stop,
   the shell returns to the three-pane library. Below ~900px the split stacks vertically.

Implementation lives in `public/caret-shell.css` + `public/operator-surface.js`. Existing DOM IDs,
output switcher labels (`슬라이드` / `전체 전사` / `회의 결과`), and WS/capture contracts stay binding.

### Style-gallery decision: Operational Liquid Glass + Caret near-black base

The installed UI/UX style gallery was queried for a premium dark operator console. The selected
primary material style remains **Liquid Glass**, now seated on a **Caret-like near-black base**
(`--caret-bg` / `--caret-panel` in `caret-shell.css`). VisionOS spatial UI, cyberpunk UI, and
generic iridescent glassmorphism stay rejected.

The product translation is **Operational Liquid Glass on a dual shell**:

- Liquid Glass supplies the material language: translucent zinc/black panels, a cool top rim,
  restrained blur, soft specular light, and short fluid state transitions.
- Caret research supplies the dual-shell information architecture: library vs compact live panel,
  detail tabs, and post-call action cards.
- Existing zinc and `--live` coral remain the accent/color story. Iridescent gallery colors,
  chromatic aberration, gradient text, and decorative glow are prohibited.
- Depth is role-based rather than decorative: stage paper is brightest, live transcript is denser,
  supporting rails recede, and the recording control floats closest to the operator.
