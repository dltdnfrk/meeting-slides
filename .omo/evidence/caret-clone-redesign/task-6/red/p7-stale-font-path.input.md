...
- Fonts ship only when the license permits redistribution. Figtree and DM Mono are vendored
  under `public/assets/fonts/` with license and SHA-256 recorded; Pretendard and system faces
  remain the Korean and fallback path. Operator surfaces never fetch a font over the network
  during tests.
