...
const server = Bun.serve({
  ...
  fetch(request, server) {
    ...
    if (url.pathname === "/control/capturing" && request.method === "POST") {
      const startedAt = Date.now() - 338000;
    ...
    if (url.pathname === "/") {
      ...
      return new Response("<!doctype html><title>Meeting Slides isolated native material QA</title><main id=\"runtime-bootstrap\">ready</main>", { headers: { "content-type": "text/html" } });
    }
  ...
  websocket: {
    open(socket) {
      sockets.add(socket); record("websocket-open", { clients: sockets.size });
      socket.send(JSON.stringify({ type: "capture", capturing: false, mode: "mic", phase: "idle", startedAt: null }));
    },
    message(_socket, message) { record("client-command", { raw: String(message) }); },
    close(socket) { sockets.delete(socket); record("websocket-close", { clients: sockets.size }); },
  },
});
record("server-ready", { port: server.port });
const fifo = Bun.file(readyFifo).writer(); fifo.write("ready\n"); await fifo.end();
