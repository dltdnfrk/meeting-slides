...
import type {
  CandidateRejection, CandidateRejectionCode, ExtractedBase, ExtractedSummary, ExtractedSummaryTopic,
  MinutesExtractionInput, MinutesExtractionResult, SourceSegmentRef,
} from "./minutes-extraction-types.ts";

export const OVERVIEW_MAX_CHARS = 600;
const TOPIC_TITLE_MAX_CHARS = 60;
const TOPIC_SUMMARY_MAX_CHARS = 300;

export function emptyResult(request: MinutesExtractionInput, batchFailed = false): MinutesExtractionResult {
  return {
    transcriptVersionId: request.transcriptVersionId,
    decisions: [], actionItems: [], openItems: [], rejections: [],
    batchFailed, usedFallback: false, summary: null,
  };
}
...
function sourceFrom(raw: Record<string, unknown>): SourceSegmentRef | null {
  const source = record(raw.sourceSegment ?? raw.source_segment ?? raw.source);
...
function sourceProvenance(
  source: SourceSegmentRef, request: MinutesExtractionInput,
...
function rejectionFor(
  ...
  const sourceCode = sourceProvenance(source, request);
...
function unwrapJsonObject(content: string): string {
...
export function parseMinutesExtractionJson(content: string, request: MinutesExtractionInput): MinutesExtractionResult {
  ...
  try { top = record(JSON.parse(unwrapJsonObject(content))); } catch { /* batch failure below */ }
  ...
  result.summary = parseExtractedSummary(top, request, result.rejections);
  return result;
}

function parseExtractedSummary(
  top: Record<string, unknown>,
  request: MinutesExtractionInput,
  rejections: CandidateRejection[],
): ExtractedSummary | null {
  const raw = record(top.summary);
  if (!raw) return null;
  const topics: ExtractedSummaryTopic[] = [];
  const values = Array.isArray(raw.topics) ? raw.topics : [];
  values.forEach((value, candidateIndex) => {
    const topic = record(value);
    const source = topic ? sourceFrom(topic) : null;
    const code = !topic || !source ? "missing_source" : sourceProvenance(source, request);
    if (code || !topic || !source) {
      rejections.push({ kind: "topic", candidateIndex, code: code ?? "missing_source" });
      return;
    }
    const title = typeof topic.title === "string" ? topic.title.trim() : "";
    const summary = typeof topic.summary === "string" ? topic.summary.trim() : "";
    if (!title || !summary) {
      rejections.push({ kind: "topic", candidateIndex, code: "missing_description" });
      return;
    }
    topics.push({
      title: title.slice(0, TOPIC_TITLE_MAX_CHARS),
      summary: summary.slice(0, TOPIC_SUMMARY_MAX_CHARS),
      source,
    });
  });
  const overviewText = typeof raw.overview === "string" ? raw.overview.trim() : "";
  const overview = (overviewText || topics.map((topic) => topic.summary).join(" ")).slice(0, OVERVIEW_MAX_CHARS);
  return overview || topics.length > 0 ? { overview, topics } : null;
}

const DECISION_END = /(?:(?:결정|확정|합의|채택)(?:했습니다|하였습니다|했다|하였다|합니다|한다)|하기로 (?:했습니다|했다|하였다)|(?:그걸로|그렇게) (?:하죠|하자)|we decided to .+)[.!?]?$/i;
