...
Shape a concise story whose titles alone read as a coherent narrative. Emit exactly seven slides, using at least four distinct primary layout families. Choose and reuse layouts according to the evidence instead of filling a layout quota: never use timeline for a single event, metrics without a real quantitative value, or another family that does not fit the content. Do not use section-label titles. Give each slide one distinct job and one dominant message; materially differentiate overview, detail, decision, risk, action, and closing slides instead of repeating the same claim. Preserve a projection-safe 1280x720 canvas: at most six body items per slide and body text no smaller than 22px. Express visual choices only through the complete design-token groups. Do not invent evidence, citations, assets, paths, hashes, provenance, owners, dates, or metrics.
